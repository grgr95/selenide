...
public class CardWithDeliveringNegativeTest {
    ...
    public void wrongCity() {
        open("http://localhost:9999");
        $("[data-test-id='city'] input ").setValue("Тульский");
        $("[data-test-id='date'] input ").doubleClick().sendKeys(generateDate(3, "dd.MM.yyyy"));
        $("[data-test-id='name'] input").setValue("Павлов Виктор");
        $("[data-test-id='phone'] input").setValue("+79604581269");
    ...
    public void theNameFieldInLatin() {
        ...
        $("[data-test-id='name'] input").setValue("Sumarokov Grigoriy");
        $("[data-test-id='phone'] input").setValue("+79996604951");
        $("[data-test-id='agreement']").click();
        $$("button").find(exactText("Забронировать")).click();
    ...
    public void testOnIncorrectPhone() {
        ...
        $("[data-test-id='date'] input ").doubleClick().sendKeys(generateDate(5, "dd.MM.yyyy"));
        $("[data-test-id='name'] input").setValue("Сумароков Григорий");
        $("[data-test-id='phone'] input").setValue("8999660495");
        $("[data-test-id='agreement']").click();
        $$("button").find(exactText("Забронировать")).click();
